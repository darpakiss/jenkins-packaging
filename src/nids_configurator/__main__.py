...
def build_parser(configurator):
    ...
    # ----- general -----
    output_default = env_get("NDIS_OUTPUT", None)
    parser.add_argument("-o", "--output", default=output_default, help="Path to save configuration file")

    non_interactive_default = env_get_bool("NDIS_NON_INTERACTIVE", False)
    parser.add_argument("--non-interactive", action="store_true", default=non_interactive_default,
                        help="Run in non-interactive mode (future extension)")

    nids_name_default = env_get("NDIS_NIDS_NAME", general["nids_name"])
    parser.add_argument("--nids-name", default=nids_name_default,
    ...
    parser.add_argument("--config-version", type=int, default=config_version_default,
    ...
    parser.add_argument("--iface", dest="interfaces", action="append", default=iface_default,
                        help="Network interface to monitor, can be used multiple times"
                             " (env: NDIS_INTERFACES, comma-separated)")

    ipv4_home_default = env_get_list("NDIS_IPV4_HOME_NETS", None)
    parser.add_argument("--ipv4-home", dest="ipv4_home_nets", action="append", default=ipv4_home_default,
                        help="IPv4 home network in CIDR, can be used multiple times"
                             "(env: NDIS_IPV4_HOME_NETS, comma-separated)")

    ipv4_excl_default = env_get_list("NDIS_IPV4_EXCLUDED_NETS", None)
    parser.add_argument("--ipv4-excl", dest="ipv4_excluded_nets", action="append",
    ...
    parser.add_argument("--ipv6-home", dest="ipv6_home_nets", action="append", default=ipv6_home_default,
                        help="IPv6 home network in CIDR, can be used multiple times"
                             " (env: NDIS_IPV6_HOME_NETS, comma-separated)")

    ipv6_excl_default = env_get_list("NDIS_IPV6_EXCLUDED_NETS", None)
    parser.add_argument("--ipv6-excl", dest="ipv6_excluded_nets", action="append",
                        default=ipv6_excl_default, help="IPv6 excluded network in CIDR, can be used multiple times "
                                                        "(env: NDIS_IPV6_EXCLUDED_NETS, comma-separated)")

    # ----- rules -----
    rule_paths_default = env_get_list("NDIS_RULE_PATHS", None)
    parser.add_argument("--rule-path", dest="rule_paths", action="append", default=rule_paths_default,
    ...
    parser.add_argument("--enable-rule-set", dest="enabled_rule_sets", action="append",
    ...
    parser.add_argument("--disable-rule-set", dest="disabled_rule_sets", action="append",
    ...
    parser.add_argument("--log-mode", choices=["file", "syslog", "both"], default=log_mode_default,
    ...
    parser.add_argument("--syslog-target", default=syslog_target_default,
    ...
    parser.add_argument("--log-level", choices=["DEBUG", "INFO", "WARNING", "ERROR"],
                        default=log_level_default, help="Logging level (env: NDIS_LOG_LEVEL)")
...
def main():
    ...
    if args.output:
        configurator.config_path = args.output
